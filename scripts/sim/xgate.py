...
# six cardinal states for average gate fidelity
def six_states():
    return [
...
# target gate: choose one
Utarget_X90 = np.array([[1, -1j], [-1j, 1]], dtype=complex) / np.sqrt(2)
Utarget_Xm90 = np.array([[1, 1j], [1j, 1]], dtype=complex) / np.sqrt(2)
Utarget = Utarget_X90
...
V = logical_projector_matrix(psi0L, psi1L)   # shape (8,2)
    ...
    for j, J23_mhz in enumerate(J23_scan_mhz):
        ...
        for delta12, delta23 in noise_samples:
            ...
            for psiL in states6:
                ...
                f_state_list.append(abs(np.vdot(psi_target, psi_log_out)) ** 2)
                l_state_list.append(max(0.0, 1.0 - np.vdot(psi_log_out, psi_log_out).real))

            f_list.append(np.mean(f_state_list))
            l_list.append(np.mean(l_state_list))

        fid_map[i, j] = np.mean(f_list)
        leak_map[i, j] = np.mean(l_list)

# theoretical X90 point from 1q effective Hamiltonian at this tau
tau_us = tau * 1e6

solJ12_coeff = ((g2 - g3) ** 2) / ((g1 + g2 - 2.0 * g3) ** 2)
hx_coeff_mhz = 1.0 / (2.0 * np.sqrt(1.0 + ((g2 - g3) ** 2) / ((g1 - g3) ** 2)))

dJ23_th_mhz = 1.0 / (8.0 * hx_coeff_mhz * tau_us)
dJ12_th_mhz = solJ12_coeff * dJ23_th_mhz
...
plt.xlabel("J23 (MHz)")
plt.ylabel("J12 (MHz)")
plt.title("nfidelity to X90 after 200 ns from idle, " \
"")
...
plt.xlabel("J23 (MHz)")
plt.ylabel("J12 (MHz)")
plt.title("Leakage after 200 ns from idle")
plt.colorbar(label="Average leakage")
plt.tight_layout()
plt.show()